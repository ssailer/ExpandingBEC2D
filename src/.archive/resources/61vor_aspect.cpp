...
const int n_x=1000,n_y=1000; //Lattice size (1000x1000)  
...
const double min_x=4,min_y=4; //Symmetric axis ranges start from -min_x and -min_y (4,4)
...
const int n_save_RTE=100; //Save RTE after every n_save_RTE iterations - intial state is auto saved (100)
const int n_it_RTE=5001; //Number of iterations for RTE (5001)
const int name=2; //Name of output (must be an integer)
...
double major,minor; //Major and minor axes of the ellipse
double ratio; //Aspect ratio of the ellipse

double a,y_a,b,y_b,c,y_c,x_0,y_0;

double dummy=0;
double Integral_aspect_aux=0;
...
complex<double> function_RTE(int i, int j, complex<double> t) //Function used for the RTE Runge-Kutta evolution
...
double Integral_1D_x()
{
        Integral_aspect_aux=0;

        for(int i=n_x/2;i<n_x-1;i++)
        { Integral_aspect_aux+=(norm(psi[i][n_y/2])+norm(psi[i+1][n_y/2]))*real(h_x)*real(lambda_x(t_abs))/2; }

        return Integral_aspect_aux;
}

double Interval_Integral_x() //Returns position on axis where integral equals 90% of the axis-integral
{
        dummy=Integral_1D_x();

        Integral_aspect_aux=0;

        for(int i_aux=n_x/2;i_aux<n_x-1;i_aux++)
        {
                a=real(x_expand(i_aux,t_abs));
                b=real(x_expand(i_aux+1,t_abs));

                if((Integral_aspect_aux+(norm(psi[i_aux +1][n_y/2])+norm(psi[i_aux][n_y/2]))*(b-a)/2)<0.9*dummy)
                { Integral_aspect_aux+=(norm(psi[i_aux +1][n_y/2])+norm(psi[i_aux][n_y/2]))*(b-a)/2; }
                else
	        {
                        y_a=norm(psi[i_aux][n_y/2]);
                        y_b=norm(psi[i_aux +1][n_y/2]);
                        y_c=(y_a+y_b)/2;
                        break;
                }
        }
        ...
        while((b-a)>0.000000001) //Nested intervals
        {
                if((Integral_aspect_aux+(y_c+y_a)*(c-a)/2)<(0.9*dummy))
                {
                        Integral_aspect_aux+=(y_c+y_a)*(c-a)/2;
                        a=c;
                        c=(a+b)/2;
                        y_a=y_c;
                        y_c=(y_a+y_b)/2;
...
double Integral_1D_y()
{
        Integral_aspect_aux=0;

        for(int j=n_y/2;j<n_y-1;j++)
	{ Integral_aspect_aux+=(norm(psi[n_x/2][j])+norm(psi[n_x/2][j+1]))*real(h_y)*real(lambda_y(t_abs))/2; }

        return Integral_aspect_aux;
}

double Interval_Integral_y() //Returns position on axis where integral equals 90% of the axis integral
{
        dummy=Integral_1D_y();
        ...
        for(int i_aux=n_x/2;i_aux<n_x-1;i_aux++)
        {
                a=real(y_expand(i_aux,t_abs));
                b=real(y_expand(i_aux+1,t_abs));

                if((Integral_aspect_aux+(norm(psi[n_x/2][i_aux +1])+norm(psi[n_x/2][i_aux]))*(b-a)/2)<0.9*dummy)
                { Integral_aspect_aux+=(norm(psi[n_x/2][i_aux +1])+norm(psi[n_x/2][i_aux]))*(b-a)/2; }
                else
                {
                        y_a = norm(psi[n_x/2][i_aux]);
                        y_b = norm(psi[n_x/2][i_aux +1]);
                        y_c = (y_a + y_b)/2;
                        break;
                }
        }
...
void Aspect_Ratio(double k)
{
        major=Interval_Integral_x();
        minor=Interval_Integral_y();
        ratio=major/minor;
        save_r(k,ratio);
}
