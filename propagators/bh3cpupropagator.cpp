...
Bh3CPUPropagator::Bh3CPUPropagator(const PathOptions &opt, const ComplexGrid &start) : Bh3Propagator(opt,start)
{
	bench_fft = bench_rprop = bench_kprop = 0.0;
	bench_fft_sys = bench_rprop_sys = bench_kprop_sys = 0.0;
	
	kprop = new ComplexGrid(opt.grid[0], opt.grid[1], opt.grid[2],opt.grid[3]);
