...
def main():
	hdf5_file_name = 'results/test08/runData/Expanding_RunEval.h5'
	group_name = '/0/Observables'
	
	file = h5py.File(hdf5_file_name, 'r')
	group = file[group_name]
	
	# dataset1 = group['KVector'][:]
	dataset = group['Contour'][:]

	samplesize = dataset[0]
	samplelength = dataset[1:samplesize+1][::1]

	offset = samplesize+1
	x = dataset[offset:samplelength[0]][::2]
	y = dataset[offset+1:samplelength[0]+1][::2]
