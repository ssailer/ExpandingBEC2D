...
public:
  ...
  bool append_snapshot(const string &name, double time,
                       const vector<ComplexGrid> &k);
  bool append_snapshot(const string &name, double time,
                       const vector<RealGrid> &k);

  bool append_averages(const string &vec_name, double *vec, int vec_dim, int* vec_rank, double time);

  bool append_docstring(const string &group, const string &docstring, double time);

  bool get_snapshot(const string &name, double time, vector<ComplexGrid> &k);
  bool get_snapshot(const string &name, double time, vector<RealGrid> &k);

  const vector<double> & get_timelist() const {return time_list;}

  const PathOptions & get_options() const {return options;}
protected:
  Bh3BinaryFile() {}
