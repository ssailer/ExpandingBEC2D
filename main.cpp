...
void initialize_psi(const int & n_x,const int & n_y, complex< double > & h_x, complex< double > & h_y){
	...
	for(i=0;i<n_x;i++) //Initialise the wavefunction
	{
		for(j=0;j<n_y;j++)
		{
		        psi[i][j]=gauss(x_axis[i],y_axis[j]); // Initialize with gaussian
			psi_copy[i][j]=psi[i][j];
			phase[i][j]=0; 
		}	
	}
...
void ITP(const int & n_x, const int & n_y, complex<double> & t_ITP){
	//k1		
...
int main( int argc, char** argv) 
{
	...
	for(i=0;i<opt.grid[1];i++){x_axis[i]=-min_x+i*real(h_x);} //Initialisation of the x-axis from -min_x to +min_x
	for(j=0;j<opt.grid[2];j++){y_axis[j]=-min_y+j*real(h_y);} //Initialisation of the y-axis from -min_y to +min_y
